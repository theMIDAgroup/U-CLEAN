...
; U-CLEAN for STIX visibilities (see Perracchione E. et al (2021) for details).

u_clean_map = stx_u_clean(vis,aux_data,clean_map[3], imsize=imsize, pixel=pixel, flare_loc = xy_flare_stix)
loadct,5,/silent
window, 0
cleanplot
plot_map, u_clean_map, /cbar,title='U-CLEAN MAP - CLEAN contour (50%)'
plot_map,clean_map[0],/over,/perc,level=[50]

stx_plot_fit_map, u_clean_map, this_window=1
...
loadct,5,/silent
window,0,xsize=7*imsize[0],ysize=5*imsize[0]
cleanplot
!p.multi=[0,3,2]
chs2=2
plot_map,bp_nat_map,charsize=chs2,title='BACK PROJECTION (nat. weight.)'
...
plot_map,u_clean_map,charsize=chs2,title='U-CLEAN'
